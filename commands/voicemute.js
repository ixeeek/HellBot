...
module.exports = {
    name: 'voicemute',
    usage: 'voicemute <@member/id> <czas/perm> [powód]',
    execute(message, args){
        const target = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
        const rsn = args.slice(2).join(' ');
        const duration = args[1];
        let role = message.guild.roles.cache.find(role => role.id === "773896151589060648");
        ...
        const userembed = new Discord.MessageEmbed()
            .setDescription(`Zostałeś wyciszony (na kanałach głosowych) na \`hellup.pl\``)
            .setColor('DARK_AQUA')
            .addFields(
